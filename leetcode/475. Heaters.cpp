...
public:
    int findRadius(vector<int>& houses, vector<int>& heaters) {
        ...
        for (int i = 0; i < houses.size(); ++i) {
            ...
            if (!isHeated) {
                int dist = INT_MAX;
                if (start >= 0 && start < heaters.size()) {
                    dist = min(dist, abs(houses[i]-heaters[start]));
                }
                if (end >= 0 && end < heaters.size()) {
                    dist = min(dist, abs(houses[i]-heaters[end]));
                }
